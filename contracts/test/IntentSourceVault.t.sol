...
contract MockXcm is IXcm {
    ...
    function weighMessage(bytes calldata) external view returns (Weight memory) {
        return Weight(0, 0);
    }
}
...
contract IntentSourceVaultTest is Test {
    ...
    function _signIntent(IIntentSource.Intent memory intent) internal view returns (bytes memory) {
    ...
    function _getIntentHash(IIntentSource.Intent memory intent) internal view returns (bytes32) {
    ...
    function test_SettleIntent_Success() public {
        ...
        assertEq(token.balanceOf(solver), 100 ether);
